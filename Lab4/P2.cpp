...
void Q1_insertToSortedList(douList* list, int data) {
    if (getLength(list) == 0 || data <= list->head->data){
        addHead(list, data);
        return;
    }
    ...
    for (int i=0; i<getLength(list)-1; i++) {
        if (data == getDataAt(list, i)) {
            addAt(list, i, data);
            return;
        } else if (data > getDataAt(list, i) && data < getDataAt(list, i+1)) {
            addAt(list, i+1, data);
            return;
        }
    }
